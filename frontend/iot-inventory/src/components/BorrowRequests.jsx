...
export function BorrowRequests({
  ...
  const [requests, setRequests] = useState([{
    ...
    purpose: 'IoT smart home automation project',
  ...
  const updateItemQuantity = (componentId, newQuantity) => {
    ...
    setEditedItems(prev => prev.map(item => item.component.id === componentId ? {
      ...item,
  ...
  return <div className="p-8">
      <div className="mb-8">
        <div className="flex items-center justify-between mb-6">
          ...
          {pendingCount > 0 && <Badge variant="default" className="bg-orange-500">
      ...
      <Card>
        <CardContent className="p-0">
          <Table>
            ...
            <TableBody>
              {filteredRequests.map(request => <TableRow key={request.id} className="cursor-pointer hover:bg-gray-50" onClick={() => openRequestDetails(request)}>
                  ...
                  <TableCell>
                    <div>
                      <div className="space-y-0.5">
                        {request.items.slice(0, 2).map((item, idx) => <div key={idx} className="font-medium">
                            {item.component.name}
                          </div>)}
                        {request.items.length > 2 && <p className="font-medium">+{request.items.length - 2} more component{request.items.length - 2 !== 1 ? 's' : ''}</p>}
                      </div>
                  ...
                  <TableCell>
                    {request.status === 'pending' && <Badge variant="secondary" className="bg-orange-100 text-orange-700">
                        <Clock className="w-3 h-3 mr-1" />
                        Pending
                      </Badge>}
                    {request.status === 'approved' && <Badge variant="default" className="bg-green-500">
                        <CheckCircle className="w-3 h-3 mr-1" />
                        Approved
                      </Badge>}
                    {request.status === 'declined' && <Badge variant="destructive">
      ...
      {filteredRequests.length === 0 && <Card className="mt-4">
          ...
          {selectedRequest && <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="mt-1">
                    {selectedRequest.status === 'pending' && <Badge variant="secondary" className="bg-orange-100 text-orange-700">
                        <Clock className="w-3 h-3 mr-1" />
                        Pending
                      </Badge>}
                    {selectedRequest.status === 'approved' && <Badge variant="default" className="bg-green-500">
                        <CheckCircle className="w-3 h-3 mr-1" />
                        Approved
                      </Badge>}
                    {selectedRequest.status === 'declined' && <Badge variant="destructive">
                        <XCircle className="w-3 h-3 mr-1" />
                        Declined
                      </Badge>}
                  </div>
              ...
              {/* Status Info */}
              {selectedRequest.status === 'approved' && <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
              ...
              {selectedRequest.status === 'declined' && <div className="p-3 bg-red-50 border border-red-200 rounded-lg">
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        {selectedRequest.status === 'pending' && <TableHead className="w-[80px] text-center">Action</TableHead>}
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {(selectedRequest.status === 'pending' ? editedItems : selectedRequest.items).map(item => <TableRow key={item.component.id}>
                          <TableCell>
                            <div>
                          ...
                          <TableCell className="text-center">
                            {selectedRequest.status === 'pending' ? <Input type="number" value={item.quantity} onChange={e => updateItemQuantity(item.component.id, parseInt(e.target.value) || 1)} className="w-16 text-center mx-auto" min="1" max={item.component.available} /> : <span className="font-medium">{item.quantity}</span>}
                          </TableCell>
                          {selectedRequest.status === 'pending' && <TableCell className="text-center">
                              <Button variant="ghost" size="sm" onClick={() => removeItemFromRequest(item.component.id)} className="h-8 w-8 p-0">
              ...
              {selectedRequest.status === 'pending' && <div>
                  ...
                  <Textarea id="declineReason" placeholder="Enter reason for declining this request..." value={declineReason} onChange={e => setDeclineReason(e.target.value)} rows={3} className="mt-1" />
                </div>}
              ...
              <div className="flex gap-3 justify-end pt-4 border-t">
                ...
                {selectedRequest.status === 'pending' && <>
                    <Button variant="destructive" onClick={declineRequest}>
                      Decline Request
                    </Button>
                    <Button onClick={approveRequest} disabled={editedItems.length === 0}>
                      Approve Request
                    </Button>
                  </>}
              </div>
