...
export function UserBorrowings() {
  ...
  const borrowings = [
    ...
    {
      id: '6',
      component: 'Temperature Sensor DS18B20',
      quantity: 2,
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card 
          className={`cursor-pointer transition-all hover:shadow-md ${
            filterType === 'all' ? 'ring-2 ring-blue-600' 
          }`}
        ...
        <Card 
          className={`cursor-pointer transition-all hover:shadow-md ${
            filterType === 'active' ? 'ring-2 ring-green-600' 
          }`}
        ...
        <Card 
          className={`cursor-pointer transition-all hover:shadow-md ${
            filterType === 'overdue' ? 'ring-2 ring-red-600' 
          }`}
        ...
        <Card 
          className={`cursor-pointer transition-all hover:shadow-md ${
            filterType === 'returned' ? 'ring-2 ring-purple-600' 
          }`}
      ...
      <div className="space-y-4">
        {filteredBorrowings.map((borrowing) => (
          <Card key={borrowing.id} className="border-l-4" style={{
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <Badge
                      variant={borrowing.status === 'overdue' ? 'destructive' : 'default'}
                      className={
                        borrowing.status === 'returned' ? 'bg-purple-500'  borrowing.status === 'active' ? 'bg-green-500' 
                      }
                    >
                ...
                {borrowing.status !== 'returned' && (
                  <div className={`text-right ${
                    borrowing.status === 'overdue' ? 'text-red-600' : 'text-blue-600'
                  }`}>
                    <p className="text-sm">
