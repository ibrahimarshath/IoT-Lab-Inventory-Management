    ...
    setDevices(devices.map(device => {
      if (device.id === id) {
        ...
        return {
          ...device,
          isOn,
          intensity ? (device.type === 'light' ? 80 ) ,
          powerUsage ? (device.type === 'light' ? 30 ) ,
          lastToggled Date().toLocaleString('en-US', { 
            year, 
            month: '2-digit', 
            day: '2-digit', 
            hour: '2-digit', 
  ...
  const updateIntensity = (id, value) => {
    setDevices(devices.map(device => {
      if (device.id === id) {
        ...
        const powerUsage = device.isOn ? Math.round((device.type === 'light' ? 45 ) * (intensity / 100)) : 0;
        return { ...device, intensity, powerUsage };
      }
      return device;
    }));
  };

  const toggleAllDevices = (type | 'fan', state) => {
    setDevices(devices.map(device => {
      if (device.type === type) {
        return {
          ...device,
          isOn,
          intensity ? (type === 'light' ? 80 ) ,
          powerUsage ? (type === 'light' ? 30 ) ,
          lastToggled Date().toLocaleString('en-US', { 
            year, 
            month: '2-digit', 
            day: '2-digit', 
